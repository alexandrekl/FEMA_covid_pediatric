...
df_plot <- bind_rows(
  ...
  filter(target_end_date > start_date #& !is.na(quant0.5)
         ) 
...
```{r, fig.height=4, fig.width=6}
daily_cases_hosp %>%
  filter(state %in% target_states & collection_date > start_date) %>% 
...
df <- read.csv( turl ) %>% tidyr::pivot_longer( cols = starts_with("X")
              ...
              mutate( date=as.Date(gsub("\\.","\\/",date), format="%m/%d/%y")
                      , location=sprintf("%05d", FIPS) ) %>%
              filter( substr(location, start=1, stop=2) %in% c(target_statefips, '36') )
...
eurl <- paste0('https://github.com/reichlab/covid19-forecast-hub/raw/master/data-processed/COVIDhub-ensemble/' 
               , date_latest_forecast, '-COVIDhub-ensemble.csv')
...
ens_fcst <- df %>%
  filter( substr(location, start=1, stop=2) %in% c(target_statefips, '36')    # New England states only
...
openxlsx::write.xlsx( filter( cumulhrrs, date >= "2021-05-30" ), fname ) 
...
openxlsx::write.xlsx( inchrrs100k, fname ) 
