...
```{r, fig.height=4}
state_d %>%
  filter( state %in% target_states & collection_date > '2022-01-20') %>% 
...
date_most_hospitals_reported <- '2022-01-26'
...
```{r}
state_d %>%
  filter( state == "MA" & collection_date > '2022-01-07') %>% 
  mutate(r = staffing_shortage_yes/total_hosp_reporting) %>%
...
```{r, fig.height=4}
region_ma %>%
  filter(fema_region %in% c("Region 1") & collection_date > '2022-01-21') %>% #& collection_date < '2020-12-25'
...
```{r}
us_ma %>%
  filter(collection_date > '2022-01-21') %>%
...
state_ma %>%
  filter(state %in% target_states & collection_date > '2022-01-21') %>%
  select(
    state
    , collection_date
    , total_utilization
